...
namespace Labirint
{
    class PathFinger
    {
        ...
        public PathFinger() 
        {
            Field = new int[n, m];//рабочее поле лабиринта
      ...
      /*56*/    {-3,-3,-3,-3,-3,-3,-3,-3,-3,-3,-3,-3,-3,-3,-3,-3,-3,-3,-3,-3,-3,-3,-3,-3,-3,-3,-3,-3,
        ...
        public void SearchPath()
        {
            ...
            do
            {
                ...
                for (int i = 0; i < m; i++)
                {
                    for (int j = 0; j < n; j++)
                    {
                        if (Field[i, j] == Ni)//если элемент поля равен номеру шага
                            //перебираем соседние клетки
                        {
                            //сверху
                            if (Field[i + 1, j] == -5)//если это финиш-конец поиска
                            {
                                Field[i + 1, j] = Ni + 1;//записываем туда значение шага
                                endOfSearch = true;//признак конца поиска
                                break;//выход из цыкла
                            }
                                else if (Field[i + 1, j] == -7)//если это коридор
                                {
                                    Field[i + 1, j] = Ni + 1;//записываем туда значение шага
                                };
                            //снизу
                            ...
                            //справа
                            if (Field[i, j + 1] == -5)//если это финиш-конец поиска
                            {
                                Field[i, j + 1] = Ni + 1;//записываем туда значение шага
                                endOfSearch = true;//признак конца поиска
                                break;//выход из цыкла
                            }
                                else if (Field[i, j + 1] == -7)//если это коридор
                                {
                                    Field[i, j + 1] = Ni + 1;//записываем туда значение шага
                                };
                            //слева
        ...
        public void MinPath()
        {
            ...
            do
            {
                ...
                if (Field[y, x] == Ni)//начинаем с финиша и перебираем соседние ячейки
                {
                    //сверху
                    ...
                    if ((Field[y + 1, x] < minID) && (Field[y + 1, x] != -1) && (Field[y + 1, x] != -3) &&
                                                     (Field[y + 1, x] != -5) && (Field[y + 1, x] != -7))
                    {
                        minID = Field[y + 1, x];//меняем значение минимального номера шага
                    ...
                    //справа
                    if ((Field[y, x + 1] < minID) && (Field[y, x + 1] != -1) && (Field[y, x + 1] != -3) &&
                                                     (Field[y, x + 1] != -5) && (Field[y, x + 1] != -7))
                    {
                        minID = Field[y, x + 1];
                        tempY = y;
                        tempX = x + 1;
                    ...
                    Field[y, x] = -1;//помечаем ячейку как маршрут
